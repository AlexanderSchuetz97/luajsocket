...
public class UDPGetPeerNameFunction extends AbstractUDPFunction{

    public UDPGetPeerNameFunction(LuaJSocketLib env) {
        super(env);
    }

    @Override
    protected Varargs invoke(UDPMaster master, Varargs args) {
        InetSocketAddress inetSocketAddress = master.getConnectedAddress();
        if (inetSocketAddress == null) {
            return varargsOf(NIL, valueOf("not connected"));
        }
